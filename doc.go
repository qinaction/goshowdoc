...
func saveDoc(param *docParam) {
	...
	if cli.DocOpen == 1 {
		...
		temDoc := "docApic"
		err = os.MkdirAll(temDoc+cli.Doc.DocDir+"/"+param.CatName, os.ModePerm)
		if err != nil {
			fmt.Println("创建文件夹失败：",err.Error())
			return
		}
		file, err := os.OpenFile(temDoc+cli.Doc.DocDir+"/"+param.CatName+"/"+param.PageTitle+".md", os.O_CREATE|os.O_APPEND|os.O_WRONLY|os.O_TRUNC, 0666)
		if err != nil {
			fmt.Println(err.Error())
			return
		}
